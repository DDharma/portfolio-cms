...
import { useExperienceList, useDeleteExperience, useSectionMetadata, useUpdateSectionMetadata } from '@/hooks/use-admin-api'
...
export default function ExperienceListPage() {
  ...
  const { data: sectionHeading, isLoading: isLoadingSectionHeading } = useSectionMetadata('experience')
          ...
          { header: 'Created', key: 'created_at', render: (_, item) => new Date(item.created_at).toLocaleDateString() },
          {
            ...
            render: (_, item) => (
              <div className="flex items-center gap-2">
                <button onClick={() => router.push(`/admin/experience/${item.id}`)} className="text-zinc-400 hover:text-white cursor-pointer">
                  <Edit className="h-4 w-4" />
                </button>
