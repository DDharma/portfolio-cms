...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: settings ? `${settings.site_name} · ${settings.site_title}` : DEFAULT_SITE_NAME,
...
export default async function RootLayout({
  ...
  try {
    ...
    const [siteSettings, stylesResult] = await Promise.all([
      getSiteSettings(),
      supabase.from('custom_styles').select('id, name, css_rules, is_active').eq('is_active', true),
    ])
