...
import { ResearchPaper, researchPaperSchema } from '@/lib/validations/research.schema'
...
import { useCreateResearch, useUpdateResearch } from '@/hooks/use-admin-api'

// Custom type for form to avoid Zod schema type inference issues
type ResearchPaperForm = {
  id?: string
  title: string
  slug: string
  description: string
  ...
  pdf_url?: string | null
  authors: string[]
  links: Array<{ id?: string; label: string; href: string; sort_order: number }>
...
export function ResearchForm({ initialData, isLoading = false }: ResearchFormProps) {
  ...
  const form = useForm<ResearchPaperForm>({
    resolver: zodResolver(researchPaperSchema as any),
  ...
  const onSubmit = async (data: ResearchPaperForm) => {
    setError(null)
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
      ...
      <div className="rounded-lg border border-white/[0.06] bg-zinc-950 p-6 space-y-6">
        ...
        <FormField label="Slug" required error={form.formState.errors.slug?.message}>
          <input
            ...
            {...form.register('slug')}
            className="w-full rounded-lg border border-white/[0.1] bg-white/[0.05] px-4 py-2.5 text-white placeholder-zinc-500 focus:border-white/[0.2] focus:outline-none"
            placeholder="paper-title (lowercase with hyphens)"
            disabled={isSubmitting || isLoading}
          />
        </FormField>
          ...
          <Controller
            ...
            render={({ field }) => (
              <RichTextEditor
                ...
                placeholder="Short abstract or description"
                disabled={isSubmitting || isLoading}
                enableCustomClasses
        ...
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1 mb-3">
            Full paper content with rich formatting support
          </p>
          <Controller
            name="content"
            control={form.control}
      ...
      <div className="rounded-lg border border-white/[0.06] bg-zinc-950 p-6 space-y-4">
        ...
        <div className="space-y-2">
          {authorsField.fields.map((field, index) => (
      ...
      <div className="rounded-lg border border-white/[0.06] bg-zinc-950 p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            variant="secondary"
            size="sm"
            onClick={() => linksField.append({ label: '', href: '', sort_order: 0 })}
            disabled={isSubmitting}
          >
            <Plus className="h-4 w-4" />
            Add Link
