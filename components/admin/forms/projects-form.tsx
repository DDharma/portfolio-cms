...
import { useCreateProject, useUpdateProject } from '@/hooks/use-admin-api'
      ...
      <div className="rounded-lg border border-white/[0.06] bg-zinc-950 p-6 space-y-6">
        ...
        <FormField label="Slug" required error={form.formState.errors.slug?.message}>
          <input
            ...
            {...form.register('slug')}
            className="w-full rounded-lg border border-white/[0.1] bg-white/[0.05] px-4 py-2.5 text-white placeholder-zinc-500 focus:border-white/[0.2] focus:outline-none"
            placeholder="project-name (lowercase with hyphens)"
            disabled={isSubmitting || isLoading}
          />
        </FormField>
        ...
        <FormField label="Thumbnail Image">
          <ImageUploader
            value={form.watch('thumbnail_image') || ''}
            onChange={(url) => form.setValue('thumbnail_image', url)}
            disabled={isSubmitting || isLoading}
          />
        </FormField>
      </div>
      ...
      <div className="rounded-lg border border-white/[0.06] bg-zinc-950 p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            variant="secondary"
            size="sm"
            onClick={() => linksField.append({ type: 'live', label: '', href: '', sort_order: 0 })}
            ...
            >
              <div className="grid grid-cols-2 gap-3">
                <select
                  ...
                  className="w-full rounded-lg border border-white/[0.1] bg-white/[0.05] px-4 py-2 text-white focus:border-white/[0.2] focus:outline-none text-sm"
                  disabled={isSubmitting}
                >
