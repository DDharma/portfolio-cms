...
export function RichTextEditor({
  ...
  const applyClassToSelection = (className: string) => {
    ...
    editor.chain().focus().setMark('customClass', { class: className }).run()
    ...
    <div className="space-y-2">
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
            !isHtmlMode ? 'bg-white/10 text-white' : 'text-zinc-400 hover:text-white'
          }`}
        >
          Visual
        </button>
      ...
      {!isHtmlMode && (
        ...
        >
          ...
          />
          {editor.isEmpty && (
            <div className="pointer-events-none absolute left-4 top-4 text-sm text-zinc-500">
              {placeholder}
            </div>
          )}
        </div>
  ...
  return (
    <button
      ...
      className={`
        ...
        ${active ? 'bg-white/[0.1] text-white' : 'text-zinc-400 hover:text-white hover:bg-white/5'}
