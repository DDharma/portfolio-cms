...
const SkillCard = ({ skill }: { skill: any }) => {
  ...
  const visibleTools = expanded ? skill.tools : skill.tools.slice(0, VISIBLE_TOOLS_COUNT)
      ...
      <div className="mt-5">
        ...
        {hasMore && (
          ...
          >
            ...
            />
            {expanded ? 'Show less' : `Show all ${skill.tools.length} tools`}
          </button>
        )}
      </div>
    </Card>
    ...
    <SectionShell id="skills">
      <SectionHeading
        ...
        description={
          ...
          'A multidisciplinary toolkit for building expressive, reliable products end-to-end.'
        }
      />
      <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3 lg:gap-6">
        {categories.map((skill: any) => {
          ...
          const transformedSkill = {
            ...
            tools: (skill.skills || [])
              .filter(
                (s: any) => s.proficiency_level === 'expert' || (s.name && s.name.includes(' '))
              )
              .map((s: any) => s.name || s),
          }
  ...
  return (
    <div className="mt-10 rounded-2xl border border-white/[0.06] p-5 lg:p-8">
      <div className="flex flex-col gap-3">
        <p className="text-xs uppercase tracking-[0.15em] text-zinc-500">Interactive</p>
        <h3 className="text-xl font-medium text-white">
          {allTags.length}+ tools and technologies across {categories.length} domains.
        </h3>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {allTags.map(({ tool, category }, index) => {
          const isActive = activeFilter === 'all' || activeFilter === category
          return (
            <span
              key={`${tool}-${index}`}
              className={cn(
